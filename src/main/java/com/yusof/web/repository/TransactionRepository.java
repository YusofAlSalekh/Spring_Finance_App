...
public interface TransactionRepository extends JpaRepository<TransactionModel, Integer> {
    @Query("select new com.yusof.web.entity.CategoryAmountModel(c.name, sum(t.amount)) " +
            "from TransactionModel t " +
            "join t.receiver s " +
            "join t.categories c " +
            "where s.clientId = :clientId and t.createdDate between :start and :end " +
            "group by c.name")
    List<CategoryAmountModel> fetchIncomeByCategory(
    ...
    @Query("select new com.yusof.web.entity.CategoryAmountModel(c.name, sum(t.amount)) " +
            ...
            "where s.clientId = :clientId and t.createdDate between :start and :end " +
            "group by c.name")
    List<CategoryAmountModel> fetchExpenseByCategory(
            @Param("clientId") int clientId,
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end);
}
